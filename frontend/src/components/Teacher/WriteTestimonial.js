...
export default function WriteTestimonial() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/mongo/teacher/testimonials`, {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        setMessage({ type: 'success', text: 'Thank you! Your testimonial has been submitted successfully.' });
    ...
    <div style={styles.container}>
      <div style={styles.content}>
        <div style={styles.header}>
          <h1 style={styles.title}>⭐ Write Testimonial</h1>
        ...
        <form style={styles.form} onSubmit={handleSubmit}>
          <div style={styles.formGroup}>
            ...
            <input type="text" value={formData.title} onChange={(e) => setFormData({...formData, title: e.target.value})} required placeholder="e.g., Great teaching platform!" style={styles.input} />
          </div>
          <div style={styles.formGroup}>
            <label style={styles.label}>Rating *</label>
            <div style={styles.ratingContainer}>
              {[1,2,3,4,5].map(star => (
                <span key={star} style={styles.star} onClick={() => setFormData({...formData, rating: star})}>
          ...
          <div style={styles.formGroup}>
            ...
            <textarea value={formData.testimonial} onChange={(e) => setFormData({...formData, testimonial: e.target.value})} required placeholder="Share your experience with Play2Learn..." style={styles.textarea} />
          </div>
          <div style={styles.formGroup}>
            <label style={styles.label}>Display Name *</label>
            <input type="text" value={formData.displayName} onChange={(e) => setFormData({...formData, displayName: e.target.value})} required style={styles.input} />
          </div>
          <div style={styles.checkbox}>
            <input type="checkbox" checked={formData.allowPublic} onChange={(e) => setFormData({...formData, allowPublic: e.target.checked})} />
            <label>Allow this testimonial to be displayed publicly</label>
          </div>
