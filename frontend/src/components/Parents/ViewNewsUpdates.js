...
function ViewNewsUpdates() {
  ...
  const fetchBroadcasts = async () => {
    try {
      ...
      if (data.success) {
        // Filter broadcasts relevant to parents
        const parentBroadcasts = (data.broadcasts || []).filter(broadcast => 
          broadcast.target_roles && 
          (broadcast.target_roles.includes('parent') || broadcast.target_roles.includes('all'))
        );
        setBroadcasts(parentBroadcasts);
      } else {
