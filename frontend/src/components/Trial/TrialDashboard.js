...
export default function TrialDashboard() {
  ...
  async function loadStudentData() {
    ...
    try {
      ...
      // ✅ Keep current selection if still exists, else fallback to first
      setSelectedStudentId((prev) => {
        if (prev && playableList.some((s) => String(s._id) === String(prev))) return prev;
        return playableList.length ? String(playableList[0]._id) : '';
      });
    } catch (e) {
      setPlayable([]);
  ...
  async function loadTeacherData(classIdOverride) {
    ...
    try {
      ...
      const classId = (classIdOverride !== undefined ? classIdOverride : '') || '';
      setSelectedClassId(String(classId));

      // ✅ If classId is empty => fetch ALL classes overview
      const url = classId
        ? `/trial/teacher/overview?classId=${encodeURIComponent(classId)}`
        : `/trial/teacher/overview`;
  ...
  useEffect(() => {
    ...
    if (mode === 'student') loadStudentData();
    else loadTeacherData(selectedClassId || undefined);
  ...
  async function submitQuiz() {
    ...
    try {
      ...
      // ✅ Refresh student list (but keep selected student)
        ...
        <table style={styles.table}>
          ...
          <tbody>
            {(rows || []).map((r, idx) => {
              ...
              return (
                <tr key={sid} style={styles.tr(sel)} onClick={() => loadTeacherStudentDetails(sid)}>
          ...
          <div style={styles.grid2}>
            <div style={styles.card}>
              ...
              {(teacherOverview.leaderboard || []).length ? (
                ...
                <div style={{ color: '#6b7280', marginTop: 10 }}>No leaderboard data yet. Let a trial student take a quiz first.</div>
              )}
            </div>
