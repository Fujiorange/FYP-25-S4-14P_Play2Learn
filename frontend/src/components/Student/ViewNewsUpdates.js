...
function ViewNewsUpdates() {
  ...
  const fetchBroadcasts = async () => {
    try {
      ...
      if (data.success) {
        // Filter broadcasts relevant to students
        const studentBroadcasts = (data.broadcasts || []).filter(broadcast => 
          broadcast.target_roles && 
          (broadcast.target_roles.includes('student') || broadcast.target_roles.includes('all'))
        );
